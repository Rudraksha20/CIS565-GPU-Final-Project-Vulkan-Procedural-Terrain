...
void Renderer::CreateRenderPass() {
	...
	// Create sampler for tex buffers
	...
	if (vkCreateSampler(logicalDevice, &sampler, nullptr, &Sampler) != VK_SUCCESS) {
...
void Renderer::CreateTexDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { grassImageLayoutBinding, skyboxImageLayoutBinding };
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // Models + TerrainGrids
        { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER , static_cast<uint32_t>(scene->GetModels().size() * 3 + scene->GetTerrainGrids().size()) },
...
void Renderer::CreateTerrainDescriptorSets() {
    // TODO: Create Descriptor sets for the terrain.
    // This should involve creating descriptor sets which point to the model matrix of each group of TerrainGrids
...
void Renderer::CreateTexDescriptorSet() {
	...
	texDescriptorGrass.sampler = Sampler;
	...
	texDescriptorSkybox.sampler = Sampler;
	...
	descriptorWrites[1].dstSet = texDescriptorSet;
	descriptorWrites[1].dstBinding = 1;
	descriptorWrites[1].dstArrayElement = 0;
	descriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	descriptorWrites[1].descriptorCount = 1;
	descriptorWrites[1].pImageInfo = &texDescriptorSkybox;
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    // The descriptors should point to Storage buffers which will hold the TerrainGrids, the culled TerrainGrids, and the output number of TerrainGrids 
    ...
    for (uint32_t i = 0; i < scene->GetTerrainGrids().size(); ++i) {
        ...
        descriptorWrites[3 * i + 0].descriptorCount = 1; // TODO: should this be 1? same for culledTerrainGrids below??? 
                                                         // I think so, because docs say descriptorCount is number of elts in pBufferInfo,
                                                         // and pBufferInfo is array of VkDescriptorBufferInfo, of which we only have one (inputTerrainGridsBufferInfo)
...
void Renderer::CreateGraphicsPipeline() {
    ...
    std::vector<VkDescriptorSetLayout> descriptorSetLayouts = { cameraDescriptorSetLayout, timeDescriptorSetLayout, texDescriptorSetLayout };
...
void Renderer::CreateTerrainPipeline() {
    ...
    VkShaderModule teseShaderModule = ShaderModule::Create("shaders/terrain.tese.spv", logicalDevice);
    ...
    std::vector<VkDescriptorSetLayout> descriptorSetLayouts = { cameraDescriptorSetLayout, modelDescriptorSetLayout, texDescriptorSetLayout, timeDescriptorSetLayout };
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        for (uint32_t j = 0; j < scene->GetModels().size(); ++j) {
            ...
            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 2, 1, &texDescriptorSet, 0, nullptr);
            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 1, 1, &timeDescriptorSet, 0, nullptr);
        ...
        for (uint32_t j = 0; j < scene->GetTerrainGrids().size(); ++j) {
            ...
            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, terrainPipelineLayout, 2, 1, &texDescriptorSet, 0, nullptr);
            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, terrainPipelineLayout, 3, 1, &timeDescriptorSet, 0, nullptr);
